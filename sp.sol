...
contract SmartProfit {
    ...
    function Bonus(uint256 _amount) public onlyOwner {
    ...
    function findFreePlace(uint256 _refId) internal returns(uint256, uint256, uint256) {
        uint256 start;
        uint256 end;
    ...
    function checkPayment(uint256 amount, uint256 rate) internal returns (uint8 status, uint256 change) {
    ...
    function findUpline(uint256 col) internal returns(uint256) {
        return uint256(col.sub(1).div(5).add(1));
    }


    function getValues(uint256 _amount) internal returns(uint256[9] memory) {
    ...
    function checkLevel(uint8 counter_, uint8 status_) internal returns (bool) {
